...
export abstract class MultiAttributeVisualization extends AVegaVisualization {
  ...
  protected addIntervalControls(attribute: IAttribute | string, axis: AxisType) {
    const attrKey = (attribute as any).dataKey ?? attribute;
  ...
  handleSplitDragEvent(name, value) {
    ...
    (this.controls.querySelector(`#split select.binType[data-axis="${axis}"]`) as HTMLSelectElement).selectedIndex = 1;
    (this.controls.querySelector(`#split input.bins[data-axis="${axis}"]`) as HTMLInputElement).value = (1 + splitValues.length).toString();
  }
