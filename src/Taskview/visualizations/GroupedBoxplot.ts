...
export class GroupedBoxplot extends MultiAttributeVisualization {
  ...
  getSpec(data: IdValuePair[]): VegaLiteSpec {
    ...
    const superSpec = super.getSpec(data) as TopLevelUnitSpec;
    ...
    const vegaLiteSpec = loMerge(superSpec, boxplotSpec) as VegaLiteSpec;

    return Object.assign(vegaLiteSpec, this.vegaLiteOptions);
  }


  generateSpec(catAttribute: IAttribute, numAttribute: IAttribute) {
    const customBrushMarks = {
      ...
      encoding: {
        ...
        x2: {field: BRUSH_DATA_END}, // no type has to be set
        color: {value: '#333'},
        opacity: {value: 0.125}
      }
    };
    ...
    const boxplotSpec: Partial<VegaLiteSpec> = {
      spacing: {row: 0},
      facet: {
        row: {
