...
export class ProbabilityScatterplot {
  ...
  handleVegaIntervalEvent(name, value) {
    ...
    const plotSelection = this.getData()
      .filter((d) => inRange(d.x, xRange[0], xRange[1]) && inRange(d.y, yRange[0], yRange[1]))
      .map((d) => d.tissuename);
    ...
    const selCol = this.lineup.data.find((d) => (d.desc as any).type === 'selection');
          ...
          on: [
            {
              events: {
                source: 'scope',
                type: 'mousedown',
                filter: ['event.ctrlKey', '!event.item || event.item.mark.name !== "brush_brush"'],
              },
          ...
          on: [
            {
              ...
              update:
                '(!isArray(brush_xAttr) || (+invert("x", brush_x)[0] === +brush_xAttr[0] && +invert("x", brush_x)[1] === +brush_xAttr[1])) && (!isArray(brush_yAttr) || (+invert("y", brush_y)[0] === +brush_yAttr[0] && +invert("y", brush_y)[1] === +brush_yAttr[1])) ? brush_scale_trigger : {}',
            },
          ],
        },
        {
