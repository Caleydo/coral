...
class Cohort(Base):
  ...
  def __repr__(self):
    return "<Cohort (id='%s', name='%s', is_initial='%s', previous_cohort='%s', entity_database='%s', entity_schema='%s', entity_table='%s', statement='%s')>" %\
           (self.id, self.name, self.is_initial, self.previous_cohort, self.entity_database, self.entity_schema, self.entity_table, self.statement)
