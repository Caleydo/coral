...
export class AColumn {
    constructor(title, $container, closeable = true) {
        ...
        this.$column = select(this.$container).append('div').classed('column', true).node();
        this.$header = select(this.$column).append('div').classed('header', true).node();
        this.$headerOptions = select(this.$header).append('div').classed('header-option', true).node();
        this.$headerTitle = select(this.$header).append('div').classed('header-title', true).node();
...
export class ADataColumn extends AColumn {
    ...
    setCohorts(cohorts) {
        ...
        this.dataCells.each(function (d, i) {
            _thisColumn.setCellStyle(select(this).node(), d, i);
        });
        //Enter
        ...
        enterSelection
            ...
            .classed('data', true)
            .each(function (d, i) {
            _thisColumn.setCell(select(this).node(), d, i);
        });
        this.dataCells.exit().remove(); // Exit
    }
    orderCohorts(cohorts) {
        this.setCohorts(cohorts);
        this.dataCells.order();
    }
    async setCell(cell, cht, index) {
    ...
    async setCellStyle(cell, cht, index) {
        ...
        if (cht.outputCohorts) { // check chohort type -> InputCohort always has a outputCohorts array
            const rows = cht.outputCohorts.length; // one row for each output cohort
            ...
            if (cht.isLastOutputCohort) {
                cell.classList.add('last-output-cohort');
                cellHeight += (10 - 2); // extra padding - default padding
                cellHeight += 1; // border
            }
            if (cht.isFirstOutputCohort) {
...
//# sourceMappingURL=AColumn.js.map