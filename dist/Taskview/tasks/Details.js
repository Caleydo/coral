...
export class Details extends ATask {
    ...
    show(container, attributes, cohorts) {
        ...
        if (cohorts.length > 0) {
            if (!this._rootChtId) {
                const bloodline = cohorts[0].getBloodline();
                this._rootChtId = bloodline[bloodline.length - 1].obj.dbId;
            ...
            this._idType = getIdTypeFromCohort(cohorts[0]);
    ...
    async addAttribute(attributes, cohorts) {
        ...
        const visibleData = this._currData.filter((row) => {
            ...
            const minOne = chtAffCols.some((elem) => elem === 'true');
            return minOne;
        });
        ...
        for (const cht of cohorts) {
            this._builder.column(LineUpJS.buildCategoricalColumn('id_' + cht.dbId, ['true', 'false']).categories([{ name: 'true', color: cht.colorTaskView }, { name: 'false', color: colors.barColor }]).label(`In: ` + cht.label));
        }
    ...
    mergeCohortData(source, target, propConflicts) {
        ...
        if (source.length === 0) {
            ...
            for (const elemT of target) {
                ...
                if (mergedData.map((mergedDataElem) => mergedDataElem[entityName]).indexOf(targetEntity) === -1) {
                    ...
                    // setColumn
                    const setCol = 'setCol';
                    const mergedSetCol = currElem[setCol];
                    currElem[setCol] = mergedSetCol.concat(elemT[setCol]);
                    // categoricalColumn
                    const catsCol = 'catsCol';
                    const mergedCatsCol = currElem[catsCol];
