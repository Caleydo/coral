...
export class Prevalence extends ATask {
    constructor() {
        super(...arguments);
        this.label = `Prevalence`;
        this.id = `prevalence`;
        this.hasOutput = false;
        this.prevalencePacks = [];
        this.excludeMissingValues = true;
        this.currentState = [];
    }
    ...
    show(columnHeader, container, attributes, cohorts) {
        ...
        for (const currPack of this.prevalencePacks) {
            ...
            if (currStateIdx !== -1) {
                ...
                const currContainer = currPack.container;
                const taskOptions = Array.from(currContainer.querySelectorAll('.ref-task-option'));
    ...
    addCheckboxForMissingValues(container) {
    ...
    createPrevalenceCohortPack(container, chtIndex, parentTaskId, cohort) {
        ...
        const chtConfig = this.createCohortAndTasksConfig(cohort, bloodline, tasks);
    ...
    createCohortAndTasksConfig(cohort, bloodline, tasks) {
        ...
        const attrVal = tasks.map((elem) => {
            ...
            const currChild = bloodline[currTaskIdx - 1].obj;
            const taskAttValue = {
                taskId: elem.id,
    ...
    addPackElements(ctrPrevPack, chtIndex, chtConfig, tasks) {
        ...
        const taskAndAttribute = tasks.map((elem) => {
        ...
        // tasks
        const dataCellsRef = select(divRefTaskContainer).selectAll('div.ref-task-option').data(taskAndAttribute, (d) => d.task.id);
        ...
        // get the styled cohort label
        const currLabel = currCht.getHTMLLabel();
        const divLegendCht = this.createLegendItem(['prev-legend-cht'], currLabel, currCht.colorTaskView);
        ...
        // tasks
        const dataCellsCht = select(divChtTaskContainer).selectAll('div.cht-task-option').data(taskAndAttribute, (d) => d.task.id);
        // create buttons for the tasks
    ...
    createLegendItem(cssClasses, label, color = null) {
        // legend item
        const divLegend = document.createElement('div');
    ...
    createBarStructure(ctrBars, colorCht) {
    ...
    createNonClickableTasks(d, chtIndex, index, nodes) {
        ...
        const valueLabel = attributes.map((attr, i) => {
            ...
            if (Array.isArray(values[i])) {
                ...
                attributeRangeLabel = values[i].map((val) => easyLabelFromFilter(val, attr.label)).join('/');
            }
            else {
                // TODO labels
                // attributeRangeLabel = labelFromFilter((values[i] as IEqualsList), attr);
                attributeRangeLabel = easyLabelFromFilter(values[i], attr.label);
            }
    ...
    createClickableTasks(d, chtIndex, index, nodes) {
        const currNode = nodes[index];
        const currTask = d.task;
        const taskRep = currTask.representation.getRepresentation();
        // add the task id as class
    ...
    addMouseEventListernersToElements(prevElem, overviewElem) {
        if (prevElem) {
            // event: mouseenter
    ...
    async handleOptionSelectionChange(chtIndex, clickedTask) {
    ...
    async updatePrevalencePack(currPack) {
        // start loading animation
        this.startBarLoadingAnimation(currPack);
        const activeTasks = select(currPack.container).selectAll('.ref-task-option.active').data().map((elem) => elem.task);
        const notActiveTasks = select(currPack.container).selectAll('.ref-task-option:not(.active)').data().map((elem) => elem.task);
        // get all task ids
    ...
    startBarLoadingAnimation(prevPack) {
        ...
        const divReference = prevPack.container.querySelector('.bar-reference.prev-value-reference');
        divReference.classList.add('loading-effect');
    }
    startTaskLoadingAnimation(clickedTask) {
        // add loading animation for the clicked task
        const cbIndicator = clickedTask.querySelector('.checkbox-indicator');
        cbIndicator.toggleAttribute('hidden', true);
        clickedTask.classList.add('color-loading');
        const cbCtrLoading = clickedTask.querySelector('.icon-container');
        cbCtrLoading.removeAttribute('hidden');
    }
    stopBarLoadingAnimation(prevPack) {
        // remove loading animation for the reference bar
        const divReference = prevPack.container.querySelector('.bar-reference.prev-value-reference');
        divReference.classList.remove('loading-effect');
    }
    stopTaskLoadingAnimation(prevPack, clickedTask = null) {
        if (clickedTask === null) {
            // remove loading animation for all tasks
            const tasks = prevPack.container.querySelectorAll('.ref-task-option');
            tasks.forEach((task) => {
                // remove loading animation of one task
                const cbCtrLoading = task.querySelector('.icon-container');
                cbCtrLoading.toggleAttribute('hidden', true);
                task.classList.remove('color-loading');
                const cbIndicator = task.querySelector('.checkbox-indicator');
    ...
    updateBars(prevPack, sizeDataset, sizeCht, sizeRef) {
        ...
        select(datasetBar).transition(tBar).style('background-color', `${colorDatasetBar}`);
        const colorDatasetMaxLabel = showDatasetBar ? colors.textColor : 'transparent';
        ...
        setTimeout(() => {
            // update ci interval after the animation for the dataset bar -> otherwise the bar size is not correct
        ...
        const scaleChtP = prevPack.container.querySelector('.scale-cohort-percentage');
...
Prevalence.EXCL_MISSING_VAL_TEXT = 'excl. missing values';
Prevalence.INCL_MISSING_VAL_TEXT = 'incl. missing values';
Prevalence.INITIAL_CHT_LABEL = 'Initial Cohort';
Prevalence.TASK_OPTION_LABEL = 'Applicable Tasks';
//# sourceMappingURL=Prevalence.js.map