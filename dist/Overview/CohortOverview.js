...
export class CohortOverview {
    ...
    executeTask(taskParam, attributes, addToTaskHistory = true) {
        log.debug('executeTask: ', taskParam);
        let validType = false;
        const parentCohort = taskParam.inputCohorts[0];
        let task;
    ...
    previewChange(taskParams, taskAttributes) {
        const tasks = [];
    ...
    generateOverview() {
        ...
        const chtOverviewGraphWrapper = document.createElement('div');
        ...
        const chtOverviewGraph = document.createElement('div');
        chtOverviewGraph.id = 'chtOverviewGraph';
        chtOverviewGraphWrapper.appendChild(chtOverviewGraph);
        // div container for grid layout and svg paths
        const chtGraph = document.createElement('div');
    ...
    setupTaskHistory() {
        ...
        const taskDetailsHeader = document.createElement('div');
        ...
        const taskDetailsStackWrapper = document.createElement('div');
        ...
        const taskDetailsStack = document.createElement('div');
        ...
        const taskHistoryHeader = document.createElement('div');
        ...
        const taskHistoryStackWrapper = document.createElement('div');
        ...
        const taskHistoryStack = document.createElement('div');
    ...
    handleHighlightingOnHover(task, taskElem, detailElem) {
        // get all paths
        const svgContainer = document.getElementById('svg-path-container');
        const pathsTaskIn = Array.from(svgContainer.querySelectorAll(`.path-ep-in-${task.id}`));
        const pathsTaskOut = Array.from(svgContainer.querySelectorAll(`.path-ep-out-${task.id}`));
    ...
    removeHighlightingFromElements(task) {
        // input cohorts
        for (const inCh of task.parents) {
    ...
    _createDetailForTaskInHistory(task) {
        ...
        // return divDetails;
        divDetails.classList.add('task-details');
        const inputs = task.parents;
        const outputs = task.children;
    ...
    _zoom() {
        const cssGrid = this.container.querySelector('div#chtGraph');
        cssGrid.style.transform = `scale(${this._zoomFactor})`;
        cssGrid.style.transformOrigin = '0% 0% 0px';
    }
}
//# sourceMappingURL=CohortOverview.js.map